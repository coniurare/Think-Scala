...
%TODO: NORMAL write a preface

\section*{Contributor List}
{\large Richard Schubert - inspiration}

%TODO: NORMAL insert contributers
...
Scala greets you and tells you its version as well as the version of the Java virtual machine on
which it is running. ``{\tt scala>}'' is the {\bf prompt} the REPL uses to indicate that it is
ready. Let's type in a very simple arithmetical expression:
...
\afterverb
%
 If you type {\tt 1 + 1}, the
REPL replies \verb#res0: Int = 2#. Here, \verb#res0# is the name of the result. You can use this
result later in other expressions. \verb#Int# is the type of the result, in this case an integer.
Anf finally \verb#2# is the result of adding 1 and 1.

\index{prompt}

Alternatively, you can store code in a text file and use the interpreter to execute the contents 
...
as a script so you can modify and execute it in the future.  To do that, you can use a simple text
editor.  However, do not use a word processor like Word or Writer, since they will not store the
text in the right format.

\section{The Scala REPL}

Let's take a moment to explain how to use the Scala REPL. The REPL has many commands that tell the
interpreter or compiler to do specific tasks, like load code from a file or exit. You can get a list of
these commands by typing \verb#:help# in the REPL. To get additional information about a command,
you can use \verb#:help COMMAND#. One particularly useful command is \verb#:quit#, which
...
But the division happens first, so you would get $\frac{\pi}{2}$, which
is not the same thing!  There is no way for Scala
to know what you meant to write, so in this case you don't
get an error message; you just get the wrong answer. To get the right result, you need to use parenthesis
around \verb#2 * pi#.
...
%TODO: LOW add int versus double division mistakes
%TODO: LOW add var/val mistakes
...
%TODO: NORMAL add reference: \footnote{You will learn more about imports and
%packages in chapter ....}
...
matching.}
%TODO: NORMAL update reference to chapter on pattern matching
